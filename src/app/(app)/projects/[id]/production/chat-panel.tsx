...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-1 text-[10px] text-[#71717a] hover:text-[#a1a1aa] transition-colors"
    >
...
function ApplyDropdown({ text, scenes, onApply }: { text: string; scenes: SceneProductionState[]; onApply: (t: ApplyTarget, text: string) => void }) {
  ...
  return (
    <div className="relative inline-block">
      ...
      {open && (
        <div className="absolute right-0 mt-1 bg-[#18181b] border border-[#27272a] rounded-lg shadow-lg z-10 py-1 w-48 max-h-48 overflow-y-auto">
          {scenes.map((s) => (
            <div key={s.sceneId} className="px-2 py-1">
              <p className="text-[10px] font-medium text-[#a1a1aa] mb-0.5">Scene {String(s.sceneOrder).padStart(2, "0")}</p>
              <button onClick={() => { onApply({ sceneId: s.sceneId, field: "klingPrompt" }, text); setOpen(false); }} className="block w-full text-left text-[11px] text-[#a1a1aa] hover:text-violet-600 hover:bg-violet-50 rounded px-1.5 py-0.5">Kling prompt</button>
              <button onClick={() => { onApply({ sceneId: s.sceneId, field: "nanoBananaPrompt" }, text); setOpen(false); }} className="block w-full text-left text-[11px] text-[#a1a1aa] hover:text-violet-600 hover:bg-violet-50 rounded px-1.5 py-0.5">Seed prompt</button>
            </div>
          ))}
        </div>
      )}
...
function MessageBubble({ msg, scenes, onApply }: { msg: Message; scenes: SceneProductionState[]; onApply: (t: ApplyTarget, text: string) => void }) {
  ...
  return (
    <div className={cn("flex", isUser ? "justify-end" : "justify-start")}>
      <div className={cn("max-w-[85%] rounded-xl px-3.5 py-2.5 text-sm leading-relaxed", isUser ? "bg-[#6366f1] text-white" : "bg-[#27272a] text-[#a1a1aa]")}>
        {/* Show attached media */}
        {msg.media?.map((att, i) => (
            ...
            return (
              <div key={i} className="my-2">
                <div className="flex items-center justify-between mb-1 gap-2">
                  <span className="text-[10px] text-[#71717a]">Suggestion</span>
                  <div className="flex items-center gap-2">
                    <CopyButton text={code} />
                    {!isUser && <ApplyDropdown text={code} scenes={scenes} onApply={onApply} />}
                  </div>
                </div>
                <pre className={cn("text-xs rounded-lg px-3 py-2 overflow-x-auto whitespace-pre-wrap", isUser ? "bg-[#27272a] text-[#a1a1aa]" : "bg-[#18181b] text-[#fafafa] border border-[#27272a]")}>{code}</pre>
              </div>
            );
          }
...
export function ChatPanel({ open, onClose, projectId, scenes, onApply }: Props) {
  ...
  return (
    <div className={cn("fixed top-0 right-0 h-full w-[420px] bg-[#18181b] border-l border-[#27272a] shadow-2xl z-50 flex flex-col transition-transform duration-300", open ? "translate-x-0" : "translate-x-full")}>
      {/* Header */}
      <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-[#27272a]">
        <div className="flex items-center gap-2.5">
          {view === "chat" && (
            <button onClick={() => setView("list")} className="p-1 rounded hover:bg-[#27272a] text-[#71717a] hover:text-[#a1a1aa] transition-colors">
          ...
          <div>
            <p className="text-sm font-medium text-[#fafafa]">Creative AI</p>
            <p className="text-[10px] text-[#71717a]">
              {view === "list" ? `${sessions.length} conversation${sessions.length !== 1 ? "s" : ""}` : `${scenes.length} scenes loaded`}
            </p>
          </div>
        </div>
        <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-[#27272a] text-[#71717a] hover:text-[#a1a1aa] transition-colors">
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      {view === "list" ? (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {loadingSessions ? (
            ...
            <div className="text-center py-12">
              <MessageSquare className="h-8 w-8 text-[#27272a] mx-auto mb-2" />
              ...
              {sessions.map((s) => (
                ...
                >
                  <button
                    ...
                    className="flex-1 text-left px-3 py-2.5 rounded-lg hover:bg-[#27272a] transition-colors"
                  >
                    <p className="text-sm text-[#a1a1aa] truncate">{s.title}</p>
                  ...
                  <button
                    ...
                    className="p-1.5 rounded hover:bg-red-500/10 text-[#52525b] hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"
                  >
        ...
        <>
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
            {messages.length === 0 && (
              <div className="text-center py-12">
                <Brain className="h-10 w-10 text-[#27272a] mx-auto mb-3" />
                <p className="text-sm font-medium text-[#a1a1aa]">Creative Collaborator</p>
                <p className="text-xs text-[#71717a] mt-1 max-w-[280px] mx-auto leading-relaxed">
                ...
                <div className="mt-4 space-y-1.5">
                  {["Rewrite scene 1's Kling prompt with more energy", "Write dialogue for a model showcasing the hoodie", "Give me 3 variations of the opening scene"].map((s) => (
                    <button key={s} onClick={() => { setInput(s); inputRef.current?.focus(); }} className="block w-full text-left text-xs text-[#a1a1aa] hover:text-violet-600 bg-[#09090b] hover:bg-violet-50 rounded-lg px-3 py-2 transition-colors">
                      {s}
                    </button>
                  ))}
                </div>
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-[#27272a] rounded-xl px-3.5 py-2.5 flex items-center gap-2">
          ...
          {/* Input */}
          <div className="shrink-0 border-t border-[#27272a] px-4 py-3 space-y-2">
            {/* Attachment previews */}
              ...
              <div className="flex gap-2 flex-wrap">
                {pendingMedia.map((att, i) => (
                  <div key={i} className="relative group/att">
                    {att.type === "image" ? (
                      <img src={`data:${att.mimeType};base64,${att.base64}`} alt={att.name} className="h-14 w-14 rounded-lg object-cover border border-[#27272a]" />
                    ) : (
                      <div className="h-14 w-14 rounded-lg bg-[#27272a] border border-[#27272a] flex items-center justify-center">
                        <Paperclip className="h-4 w-4 text-[#71717a]" />
                      </div>
                    )}
                    <button
                      onClick={() => setPendingMedia((prev) => prev.filter((_, j) => j !== i))}
                      className="absolute -top-1 -right-1 p-0.5 rounded-full bg-[#6366f1] text-white opacity-0 group-hover/att:opacity-100 transition-opacity"
                    >
                      <X className="h-2.5 w-2.5" />
                    </button>
                    <p className="text-[9px] text-[#71717a] truncate w-14 mt-0.5">{att.name}</p>
                  </div>
                ))}
              </div>
            )}
            <div className="flex gap-2">
              ...
              <button
                ...
                className="shrink-0 w-10 h-10 rounded-lg border border-[#27272a] hover:bg-[#27272a] text-[#71717a] hover:text-[#a1a1aa] flex items-center justify-center transition-colors self-end"
              ...
              <textarea
                ...
                className="flex-1 text-sm rounded-lg border border-[#27272a] px-3 py-2 bg-[#18181b] resize-none focus:outline-none focus:ring-2 focus:ring-violet-200 focus:border-violet-300 transition-all placeholder:text-[#71717a]"
                disabled={loading}
              />
              <button
                onClick={handleSend}
